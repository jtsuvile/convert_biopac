#### PART 2 ####
library(plotly)
source('/Users/juusu53/Documents/projects/femg/Preprocess-fEMG/preprocess_femg_functions.R')
...
# [1] Kunecke, J. et al. (2014) 'Facial EMG responses to emotional expressions are related to
...
labelled.data.file <- paste0('/Users/juusu53/Documents/projects/femg/data/preprocessed/',subid,'_labelled.csv')
...
# plot one of them
flagged.femg.data %>% 
  plot_flagged_trial(flaggedTrial = 33, prefixes, win.sec, prestim.sec)
...
for (t in trials$flaggedOnAnyMuscle) { 
  thisplot <- flagged.femg.data %>%
    ...
    ggplotly()
  filename <- paste0('/Users/juusu53/Documents/projects/femg/data/preprocessed/',subid,t,'.html')
...
for (muscle in prefixes) {
  pp.femg.data[[muscle]] %>% 
    write_csv(paste0('/Users/juusu53/Documents/projects/femg/data/preprocessed/',subid, '_',muscle,'_preprocessed.csv'))
}
...
for (muscle in prefixes) {
  summarised.femg.data[[muscle]] %>% 
    write_csv(paste0('/Users/juusu53/Documents/projects/femg/data/preprocessed/',subid, '_',muscle,'_means.csv'))
}
